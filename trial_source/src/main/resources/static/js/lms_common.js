...
/**
 * テーブル内で指定された行の出退勤時刻を定時（9:00 - 18:00）に設定する
 * @author 吉田知生 - Task.28
 * @param {number} index - 変更対象の行のインデックス
 */
function addTime(index) {
	// 出勤時間（Hour）のselect要素を取得
	const startTimeHourElement = document.getElementById("startTime-" + index);

	// 出勤時間（Minute）のselect要素を取得
	const startTimeMinuteElement = document.getElementById("startMinute-" + index);

	// 退勤時間（Hour）のselect要素を取得
	const endTimeHourElement = document.getElementById("endTime-" + index);

	// 退勤時間（Minute）のselect要素を取得
	const endTimeMinuteElement = document.getElementById("endMinute-" + index);

	// 出勤時間：9時を設定
	if (startTimeHourElement) {
		startTimeHourElement.value = '9';
	}

	// 出勤時間：00分を設定
	if (startTimeMinuteElement) {
		startTimeMinuteElement.value = '0';
	}

	// 退勤時間：18時を設定
	if (endTimeHourElement) {
		endTimeHourElement.value = '18';
	}

	// 退勤時間：00分を設定
	if (endTimeMinuteElement) {
		// '00' (文字列) を設定
		endTimeMinuteElement.value = '0';
	}
}